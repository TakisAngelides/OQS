...
l_0_small_list = [0.001, 0.01, 0.1, 1]
Omega_list = [0.001, 0.01, 0.1, 1]
omega_list = [0.001, 0.01, 0.1, 1]
...
mem = 8
cpu = 4
project_number = os.getcwd().strip().split('/')[-1] # <==================================== Important to change according to which project file ================================

def write_dag_static_field():
    
    sub_file_name = 'OQS_ATD_DMRG_static_field.sub'
    name_of_dag = 'OQS_ATD_DMRG_static_field'
                                                                    ...
                                                                    for omega in omega_list:
                                                                        for Omega in Omega_list:
                                                                            ...
                                                                            if tau_previous == 0:
                                                                                ...
                                                                                h5_previous_path = f'{path_to_project}/DAGS/{project_number}/HDF5/N_{N}_tau_{tau_previous}_x_{x}_l0_{l_0}_ma_{ma}_env_{env_corr_type}_sig_{sigma_over_a}_aT_{aT}_lam_{lambd}_aD0_{aD_0}_l0init_{l_0_initial_state}_cutoff_{cutoff}_l0s_{l_0_small}_Om_{Omega}_om_{omega}.h5'
                                                                            h5_path = f'{path_to_project}/DAGS/{project_number}/HDF5/N_{N}_tau_{tau}_x_{x}_l0_{l_0}_ma_{ma}_env_{env_corr_type}_sig_{sigma_over_a}_aT_{aT}_lam_{lambd}_aD0_{aD_0}_l0init_{l_0_initial_state}_cutoff_{cutoff}_l0s_{l_0_small}_Om_{Omega}_om_{omega}.h5'
                                                                            ...
                                                                            job_name = f'N_{N}_tau_{tau_text}_x_{x_text}_l0_{l_0_text}_ma_{ma_text}_env_{env_corr_type}_sig_{sigma_over_a_text}_aT_{aT_text}_lam_{lambd_text}_aD0_{aD_0_text}_l0init_{l_0_initial_state_text}_cutoff_{cutoff}_l0s_{l_0_small_text}_Om_{Omega_text}_om_{omega_text}'
                                                                            f.write(f'JOB ' + job_name + f' {path_to_project}/{sub_file_name}\n')
                                                                            f.write(f'VARS ' + job_name + f' N="{N}" tau="{tau}" cutoff="{cutoff}" tol="{tol}" x="{x}" l_0="{l_0}" ma="{ma}" max_steps="{max_steps}" project_number="{project_number}" h5_path="{h5_path}" measure_every="{measure_every}" h5_previous_path="{h5_previous_path}" D="{D}" lambda="{lambd}" aD_0="{aD_0}" aT="{aT}" sigma_over_a="{sigma_over_a}" env_corr_type="{env_corr_type}" max_sweeps="{max_sweeps}" sparse_evol="{sparse_evol}" path_to_project="{path_to_project}" file_to_run="{file_to_run}" l_0_initial_state="{l_0_initial_state}" dirac_vacuum_initial_state="{dirac_vacuum_initial_state}" max_rho_D="{max_rho_D}" l_0_small="{l_0_small}" Omega="{Omega}" omega="{omega}"\n')
                                                                            f.write('RETRY ' + job_name + ' 3\n')
                                                                            counter += 1
                                                                            ...
                                                                            if tau_previous != 0:
                                                                                ...
                                                                                tau_previous_text = str(tau_previous).replace('.', '')
                                                                                previous_job_name = f'N_{N}_tau_{tau_previous_text}_x_{x_text}_l0_{l_0_text}_ma_{ma_text}_env_{env_corr_type}_sig_{sigma_over_a_text}_aT_{aT_text}_lam_{lambd_text}_aD0_{aD_0_text}_l0init_{l_0_initial_state_text}_cutoff_{cutoff}_l0s_{l_0_small_text}_Om_{Omega_text}_om_{omega_text}'
                                                                                f.write(f'PARENT ' + previous_job_name + ' CHILD ' + job_name + '\n')
...
def make_plots_pulse_field():
    ...
    if max(N_list) <= 8:
        columns = ['N', 'x', 'ma', 'l_0_init', 'l_0', 'tau', 'env', 'sig', 'aT', 'lam', 'aD_0', 'at', 'D', 'ee', 'pn', 'l_0_small', 'omega', 'Omega', 'cutoff']
    else:
        columns = ['N', 'x', 'ma', 'l_0_init', 'l_0', 'tau', 'env', 'sig', 'aT', 'lam', 'aD_0', 'at', 'D', 'pn', 'l_0_small', 'omega', 'Omega', 'cutoff']
    df = pd.DataFrame(columns = columns)
    ...
    for filepath in os.listdir(f'{path_to_project}/DAGS/{project_number}/HDF5'):
        ...
        try:
            ...
            if row[-2] != 'om':
                continue
            N, tau, x, l_0, ma, env, sig, aT, lam, aD_0, l_0_init, cutoff, l_0_small, Omega, omega = row[1], row[3], row[5], row[7], row[9], row[11], row[13], row[15], row[17], row[19], row[21], row[23], row[25], row[27], row[29]
            N = int(N)
            tau = float(tau)
            x = float(x)
            l_0 = float(l_0)
        ...
        plt.ylabel('Maximum bond dimension')
        plt.xlabel('Iteration')
        plt.title(f'N = {N}, tau = {tau}, x = {x}, l_0 = {l_0}, ma = {ma}, env = {env}, sig = {sig}, aT = {aT}, lam = {lam}, aD_0 = {aD_0}, omega = {omega}, Omega = {Omega}, l_0_small = {l_0_small}')
        plt.savefig(f'{path_to_project}/DAGS/{project_number}/Plots/D_vs_iteration/{filepath}.png', bbox_inches = 'tight')
        plt.close()
        ...
        for i in range(len(at_list)):
            ...
            if max(N_list) <= 8:
                at, D, ee, pn = at_list[i], max_bond_list[i], ee_list[i], particle_number_density[i]
                new_row = pd.DataFrame([[N, x, ma, l_0_init, l_0, tau, env, sig, aT, lam, aD_0, at, D, ee, pn, l_0_small, omega, Omega, cutoff]], columns = df.columns)
            else:
                at, D, pn = at_list[i], max_bond_list[i], particle_number_density[i]
                new_row = pd.DataFrame([[N, x, ma, l_0_init, l_0, tau, env, sig, aT, lam, aD_0, at, D, pn, l_0_small, omega, Omega, cutoff]], columns = df.columns)
            df = pd.concat([df, new_row], ignore_index = True)
