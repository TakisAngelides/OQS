...
cutoff_list = [1e-16] # this is for compression after gate application
...
max_rho_D = 200
project_number = 13 # <==================================== Important to change according to which project file ================================
...
def make_plots():
    ...
    if max(N_list) <= 8:
        columns = ['N', 'x', 'ma', 'l_0_init', 'l_0', 'tau', 'env', 'sig', 'aT', 'lam', 'aD_0', 'at', 'D', 'ee', 'pn']
    else:
        columns = ['N', 'x', 'ma', 'l_0_init', 'l_0', 'tau', 'env', 'sig', 'aT', 'lam', 'aD_0', 'at', 'D', 'pn']
    ...
    for filepath in os.listdir(f'{path_to_project}/DAGS/{project_number}/HDF5'):
        ...
        for i in range(len(at_list)):
            ...
            if max(N_list) <= 8:
                ...
                new_row = pd.DataFrame([[N, x, ma, l_0_init, l_0, tau, env, sig, aT, lam, aD_0, at, D, ee, pn]], columns = df.columns)
            else:
                at, D, pn = at_list[i], max_bond_list[i], particle_number_density[i]
                new_row = pd.DataFrame([[N, x, ma, l_0_init, l_0, tau, env, sig, aT, lam, aD_0, at, D, pn]], columns = df.columns)
            ...
            for l_0 in df.l_0.unique():
                for ma in df.ma.unique():
                    for aD_0 in df.aD_0.unique():
                        ...
                        df_tmp = df[(df.N == N) & (df.x == x) & (df.l_0 == l_0) & (df.ma == ma) & (df.aD_0 == aD_0)]
                                                                    
                        at_list, pn_list = np.array(df_tmp['at']), np.array(df_tmp.pn)
                                                                                       
                        plt.plot(at_list, pn_list, label = f'N = {N}, aD_0 = {aD_0}')
                        plt.title('PND vs at')
