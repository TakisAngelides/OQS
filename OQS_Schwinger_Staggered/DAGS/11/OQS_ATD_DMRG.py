...
N_list = [8]
tau_list = [0.0001, 0.00001] # time step size
...
max_steps_list = [1000, 10000] # how many ATDDMRG time steps to do, this needs to be the same size as the tau list
measure_every_list = [1, 1] # how often to measure observables and store the density matrix, this needs to be the same size as the max_steps_list
cutoff_list = [1e-18] # this is for compression after gate application
tol_list = [1E-16] # tol for dmrg stopping condition
x_list = [np.round(np.pi/3, 6)] # [np.round(1/0.8**2, 6)]
ma_list = [4.5]
l_0_list = [0] 
...
project_number = 11 # <==================================== Important to change according to which project file ================================
...
def make_plots():
    ...
    if max(N_list) <= 8:
        columns = ['N', 'x', 'ma', 'l_0_init', 'l_0', 'tau', 'env', 'sig', 'aT', 'lam', 'aD_0', 'step', 'D', 'ee', 'pn']
    else:
        columns = ['N', 'x', 'ma', 'l_0_init', 'l_0', 'tau', 'env', 'sig', 'aT', 'lam', 'aD_0', 'step', 'D', 'pn']
    ...
    for filepath in os.listdir(f'{path_to_project}/DAGS/{project_number}/HDF5'):
        ...
        for i in range(len(step_num_list)):
            ...
            if max(N_list) <= 8:
                step, D, ee, pn = step_num_list[i], max_bond_list[i], ee_list[i], particle_number_density[i]
                new_row = pd.DataFrame([[N, x, ma, l_0_init, l_0, tau, env, sig, aT, lam, aD_0, step, D, ee, pn]], columns = df.columns)
            else:
                step, D = step_num_list[i], max_bond_list[i]
                new_row = pd.DataFrame([[N, x, ma, l_0_init, l_0, tau, env, sig, aT, lam, aD_0, step, D, pn]], columns = df.columns)
