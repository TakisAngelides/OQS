...
N_list = [10]
tau_list = [10**(-pow) for pow in [2, 3, 4]] # time step size
...
max_steps_list = [10, 100, 1000] # how many ATDDMRG time steps to do, this needs to be the same size as the tau list
measure_every_list = [1, 1, 1] # how often to measure observables and store the density matrix, this needs to be the same size as the max_steps_list
cutoff_list = [1e-2, 1e-4, 1e-6, 1e-8, 1e-10] # this is for compression after gate application
l_0_list = [0.9]
tol_list = [1E-16] # tol for dmrg stopping condition
x_list = [1.05] # [np.round(1/0.8**2, 6)]
ma_list = [4.5]
...
def make_plots():
    ...
    if max(N_list) <= 8:
        ...
        columns = ['N', 'x', 'ma', 'l_0_init', 'l_0', 'tau', 'env', 'sig', 'aT', 'lam', 'aD_0', 'step', 'D']
    ...
    for filepath in os.listdir(f'{path_to_project}/DAGS/{project_number}/HDF5'):
        ...
        for t in range(len(step_num_list)):
            particle_number_density.append(0.5 + (0.5/N)*sum([z_list[idx][t]*(-1)**(idx-1) for idx in range(N)]))
        ...
        for i in range(len(step_num_list)):
            ...
            if max(N_list) <= 8:
                ...
                new_row = pd.DataFrame([[N, x, ma, l_0_init, l_0, tau, env, sig, aT, lam, aD_0, step, D]], columns = df.columns)
