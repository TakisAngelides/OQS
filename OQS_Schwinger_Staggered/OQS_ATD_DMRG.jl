...
function run_ATDDMRG_static_field() 
...
function run_ATDDMRG_time_varying_field()
...
if length(ARGS) == 23 # static field case
    ...
    run_ATDDMRG_static_field()
    close(file)

    println("Finished")
    flush(stdout)

else # time varying pulse electric field case
    ...
    run_ATDDMRG_time_varying_field()
    close(file)
