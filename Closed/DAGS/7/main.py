...
def write_dag():
    ...
    lambd = 0
    number_of_time_steps_list = [500, 2000, 2000] # needs same length as tau list
    tau_list = [0.1, 0.01, 0.1]
    x_list = [10]
    ma_list = [0.0, 1.0]
        ...
        for x in x_list:
                for ma in ma_list:
                    for tau_idx, tau in enumerate(tau_list):
                        ...
                        for cutoff in [1e-7, 1e-9]:
                            for taylor_expansion_order in [3]:
    ...
    for file in os.listdir(path_to_HDF5):
        ...
        try:
            ...
            if attributes_dict['wis'] == 'dirac_vacuum_with_string':
                ...
                plt.plot(-pn + pn_without_string + pn[0], label = "subtracted")
                plt.title(f'N_{N}_x_{x_val}_ma_{ma}_qn_{cqns}\nc_{cutoff}_l01_{l_0_1}_tau_{tau}_taylor_{teo}_waf_{waf}')
                plt.legend()
                ...
                for i in range(N):
                    if i < 40 or i > 60:
                        continue
                    plt.plot(abs(z_configs[i, :] - z_configs_without_string[i, :]), label = f'Site = {i}')
                plt.legend()
                plt.title(f'N_{N}_x_{x_val}_ma_{ma}_qn_{cqns}\nc_{cutoff}_l01_{l_0_1}_tau_{tau}_taylor_{teo}_waf_{waf}')
