...
# Checking which job ids belong to which parameters
...
let
    ...
    for job_id in collect(keys(f))
        ...
        N, ma, aD, cutoff, wis, l_0_1, x = inputs["N"], inputs["ma"], inputs["aD"], inputs["cutoff"], inputs["wis"], inputs["l_0_1"], inputs["x"]
        if wis == "dirac_vacuum" # this is always 1 before
            continue   
        end
        if ma == 0.25 && aD == 2.0 && l_0_1 == 0.0
            counter += 1
            println(counter, ": ", job_id)
            push!(res, parse(Int, job_id))
        end
    end
    display(sort(res))
end
