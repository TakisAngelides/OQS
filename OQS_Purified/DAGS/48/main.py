...
def write_dag():
    ...
    tau_list = [0.05]
    tmp_aD_list = np.linspace(2.0, 5.0, 20)
    aD_list = [tmp_aD_list[0], tmp_aD_list[10], tmp_aD_list[-1]] # np.linspace(2.0, 5.0, 20)
    x_list = [1.0]
    ma_list = np.linspace(0.1, 1.0, 20)
                        ...
                        for tau_idx, tau in enumerate(tau_list):
                            ...
                            for cutoff in [1e-11]:
                                for taylor_expansion_order in [2]:
                                    tmp_l_0_1_list = np.linspace(0.0, 0.5, 20)
                                    for l_0_1 in [tmp_l_0_1_list[0], tmp_l_0_1_list[10], tmp_l_0_1_list[-1]]: # np.linspace(0.0, 0.5, 20): # this is the constant part of the applied field
    ...
    for file in os.listdir(path_to_HDF5):
        ...
        try:
            ...
            if attributes_dict['wis'] == 'dirac_vacuum_with_string':
                ...
                with open(f'Plots/Thermalization_Heatmap/{ma}_{l_0_1}_{aD}.pickle', 'wb') as f:
