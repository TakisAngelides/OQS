...
def write_dag():
    ...
    lambd = 0
    number_of_time_steps_list = [700] # needs same length as tau list
    x_list = [1/(0.5)**2]
    ma_list = [0.0]
    tau_list = [0.01]
                        ...
                        for taylor_expansion_order in [4]:
                            for l_0_1 in [0]: # this is the constant part of the applied field
                                for conserve_qns in ["true"]:
                                    for which_initial_state in ["dirac_vacuum"]: # options are: "dirac_vacuum", "gs_naive", "dirac_vacuum_with_string"
                                
                                        # Memory, CPU and maximum number of days to run
                                        mem, cpu, days = 8, 8, 6.99
