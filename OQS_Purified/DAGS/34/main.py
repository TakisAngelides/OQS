...
def write_dag():
    ...
    lambd = 0
    number_of_time_steps_list = [100] # needs same length as tau list
    tau_list = [0.005]*len(number_of_time_steps_list)
    aD_list = [5.0] # np.linspace(2.0, 5.0, 20)
    x_list = [1.0]
    ma_list = [1.0] # [0.1, 0.25, 0.5, 0.75, 1.0]
    ...
    for file in os.listdir(path_to_HDF5):
        ...
        try:
            ...
            if attributes_dict['wis'] == 'dirac_vacuum_with_string':
                ...
                plt.plot(x, z_configs[1, :], label = f'{0}') #, Max = {max(z[i, :]):.5f}
                plt.plot(x, z_configs[-2, :], label = f'{-1}') #, Max = {max(z[i, :]):.5f}
                plt.legend()
                plt.title(f'N_{N}_x_{x_val}_ma_{ma}_aD_{aD}_aT_{aT}_qn_{cqns}\nc_{cutoff}_l01_{l_0_1}_tau_{tau}_taylor_{teo}_waf_{waf}')
