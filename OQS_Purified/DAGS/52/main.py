...
def write_dag():
    ...
    tau_list = [0.05]
    aD_list = [5] # np.linspace(2.0, 5.0, 20)
    x_list = [1.0]
    ma_list = [1.0]
    ...
    for N in [12]:
        ...
        for aT in [10]: # np.linspace(7, 100, 5):
            for x in x_list:
                for ma in ma_list:
                    for aD in aD_list:
                        for tau_idx, tau in enumerate(tau_list):
                            ...
                            for cutoff in [1e-11]:
                                for taylor_expansion_order in [2]:
                                    for l_0_1 in [0.5]: # np.linspace(0.0, 0.5, 20): # this is the constant part of the applied field
...
def plot_subtracted_observables_gs_first_naive():
    ...
    for file in os.listdir(path_to_HDF5):
        ...
        try:
            ...
            if attributes_dict['wis'] == 'gs_naive':
                ...
                plt.plot(x, -pn + pn_without_string, label = f"subtracted, max = {max(-pn + pn_without_string + pn[0] - pn_without_string[0])}")
                plt.title(f'N_{N}_x_{x_val}_ma_{ma}_aD_{aD}_aT_{aT}_qn_{cqns}\nc_{cutoff}_l01_{l_0_1}_tau_{tau}_taylor_{teo}_waf_{waf}')
                plt.legend()
                plt.ylabel('Particle number')
                plt.xlabel(r'$t/a$')
...
plot_subtracted_observables_gs_first_naive()
