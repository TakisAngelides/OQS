...
def write_dag():
    ...
    lambd = 0
    number_of_time_steps_list = [400, 400, 500, 3000, 3000, 3000] # needs same length as tau list
    tau_list = [0.01]*len(number_of_time_steps_list)
    aD_list = [0.01, 0.15, 0.3, 0.4, 0.5, 0.6]
    gc = 100
    x_list = [1/(0.5)**2]
    ma_list = [0.0]
    ...
    for file in os.listdir(path_to_HDF5):
        ...
        try:
            ...
            if attributes_dict['wis'] == 'dirac_vacuum_with_string':
                ...
                plt.plot(x, -pn + pn_without_string + pn[0], label = "subtracted")
                plt.title(f'N_{N}_x_{x_val}_ma_{ma}_aD_{aD}_aT_{aT}_qn_{cqns}\nc_{cutoff}_l01_{l_0_1}_tau_{tau}_taylor_{teo}_waf_{waf}')
                plt.legend()
    ...
    for file in os.listdir(path_to_HDF5):
        ...
        try:
            ...
            if attributes_dict['wis'] == 'dirac_vacuum_with_string':
                ...
                t_over_a_list = [0] + list(tau*(np.arange(1, z_configs.shape[1])))
                x = np.round(t_over_a_list, decimals = 3)

                z_configs_diff = z_configs_without_string - z_configs
                            
                for i in range(N):
                    
                    tmp = z_configs_diff[i, :]
                    plt.plot(t_over_a_list[-500:], abs(tmp - tmp[-1])[-500:], label = f'Site = {i}')
                
                plt.yscale('log')
                plt.ylabel('abs((Z_no_string - Z_with_string) - final_value)')
                plt.xlabel('t/a')
                ma, aD, aT, cqns, cutoff, l_0_1, teo, waf, x_val = attributes_dict['ma'], attributes_dict['aD'], attributes_dict['aT'], attributes_dict['cqns'], attributes_dict['cutoff'], attributes_dict['l_0_1'], attributes_dict['teo'], attributes_dict['waf'], np.round(attributes_dict['x'], decimals = 3)
                plt.title(f'N_{N}_x_{x_val}_ma_{ma}_aD_{aD}_aT_{aT}_qn_{cqns}\nc_{cutoff}_l01_{l_0_1}_tau_{tau}_taylor_{teo}_waf_{waf}')
                plt.legend()
