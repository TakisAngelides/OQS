...
# Parameters
base_paths = {
    24: "/Users/takisangelides/Downloads/data_42",
    12: "/Users/takisangelides/Downloads/data_47"
}
masses = [0.1, 1.0]
l_0_values = [0.0, 0.5]
aD_values = [2.0, 5.0]
    ...
    for j, mass in enumerate(masses):
        for k, l_0 in enumerate(l_0_values):
            for l, aD in enumerate(aD_values):
                ...
                ax.text(0.005, 0.02, f"{letters[counter]}", color="white", ha="left", va="bottom", transform=ax.transAxes, fontsize=18)
                counter += 1

                # Configure axis tick labels only for first column and last row
                if col == 0:
                    ax.set_ylabel(r'$L$', fontsize=24)
